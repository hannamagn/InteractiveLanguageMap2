...
def call_wikidata_api(query, output_file, timeout, dump_to_file):
    ...
    if dump_to_file:
        with open(f"databaseCollector/debug/{output_file}.json", "w", encoding="utf-8") as f:
...
def get_lang_metadata(lang_data):
    ...
    for iso_list in chunked_iso_codes:
        ...
        query = f'''
        SELECT ?language ?languageLabel ?iso_code ?region ?regionLabel ?country ?countryLabel ?region_osm_id ?country_osm_id ?isOfficial ?country_of_regionLabel WHERE {{
        ...
        #OLD QUERY WITHOUT COUNTRY OF REGION
        # query = f'''
    ...
    with open(f"databaseCollector/debug/langmetadata.json", "w", encoding="utf-8") as f:
...
def call_nomimantim_api(osmid):
    ...
    with open("databaseCollector/debug/raw4.geojson", "w", encoding="utf-8") as f:
...
def get_regions(lang_data):
    ...
    with open(f"databaseCollector/debug/minifiedFormattedRegionData.geojson", "w", encoding="utf-8") as f:
...
def format_response(response):
    ...
    # with open("databaseCollector/debug/formattedTest1.geojson", "w", encoding="utf-8") as f:
